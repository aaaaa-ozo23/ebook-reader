...
- TXT 分页以舞台 frame 的真实内容尺寸测量；Double 需要两个至少 320px 页槽和实际列间距，窄窗只降级 rendered 模式，不覆盖 requested Double。TXT 与 EPUB 共用两层分页底栏结构，但 TXT 页号只映射当前布局边界，持久进度仍为 `chapterId + charOffset`。
- TXT 冷分页先整段测量，只有溢出段才生成字素边界；缓存切片用页/块双游标线性重建，测量 DOM 复用公共前缀和末尾文本节点。部分结果只用于提前显示，不写入 `txt_pagination_v1`；完整边界才进入磁盘缓存，会话内最多保留两个布局。
